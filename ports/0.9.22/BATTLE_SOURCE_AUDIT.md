...
root.  It fails when the port gains an undocumented Python module, a reviewed
...
reproducible from `ports/0.9.22` while 0.8.2 continues to evolve.  New 0.8.2
...
| `lan_session.py` | Queue/UI adapter | Retains server-owned join/start state; connects it to the #1513 lobby and battle lifecycle. A Bot-observation relay is routed only to the runtime for its active round. |
...
| `projectile_manager.py` | Bounded active-projectile owner | Owns the capped deterministic active set, elapsed advancement, progress cursors, terminal removal and server-snapshot restoration without performing native queries itself. |
| `projectile_runtime.py` | Pure trajectory/collision law | Advances every shell along one gravity parabola in chords no longer than 25 ms. Static/destructible hits and relative moving-vehicle sweeps share chronological ordering, so a target may dodge after launch. |
...
player and authority-Bot shell. It evaluates one gravity parabola in chords no
longer than 25 ms and transforms a vehicle's start/end pose into a relative
sweep for the same interval. `projectile_manager.py` owns the bounded active
