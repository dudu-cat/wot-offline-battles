...
# installer still verifies the orientation against the map bases, so a route
# cannot be silently reversed by an annotation stroke drawn in the other
# direction.
...
# farthest from the direct base line.
...
def _distance_to_base_line(point, start, end):
...
def _orient_and_mark(points, own_base, enemy_base, gate_index=None):
	...
	forward = (_distance_squared(values[0], own_base) +
	           _distance_squared(values[-1], enemy_base))
	reverse = (_distance_squared(values[-1], own_base) +
	           _distance_squared(values[0], enemy_base))
	if reverse < forward:
	...
	if gate_index is None:
		gate_index = max(
			range(len(values)),
			key=lambda index: _distance_to_base_line(
				values[index], own_base, enemy_base),
		)
...
def apply_reviewed_routes(tactical_maps):
	"""Replace reviewed route geometry while preserving tactical metadata."""
	for map_name, route_points in REVIEWED_ROUTE_POINTS.items():
		if map_name not in tactical_maps:
			raise ValueError('reviewed tactical map is unavailable: %s' % map_name)
		original = tactical_maps[map_name]
		updated = dict(original)
		bases = original.get('bases', {})
		if 1 not in bases or 2 not in bases:
			raise ValueError('reviewed tactical map has no team bases: %s' % map_name)
		...
		for route_id, points in route_points.items():
			...
			team_one[route_id] = _orient_and_mark(
				points, bases[1], bases[2], gate_index)
		new_routes = {}
		for team in (1, 2):
			converted = []
		...
		tactical_maps[map_name] = updated
