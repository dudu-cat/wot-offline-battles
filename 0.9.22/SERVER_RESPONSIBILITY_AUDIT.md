...
## Version 0.4.0 product boundary

The top-level `0.9.22` layout, `127.0.0.1:28782` release default and separate
`server_endpoint.json` user file are deployment and local-configuration
changes. They do not move room, map, team, health or combat authority. Editing
the endpoint in the stock settings surface changes only the next LAN
connection and never promotes that player to room host.

Suppressing the exact CN automatic server-announcement call is local
presentation. The adapter acts only at that stock `onLobbyInited` boundary
before browser creation and leaves explicit `showBrowser` calls available. The
plain-language waiting-room text likewise changes no server admission rule.

The double-click Windows launcher packages the same Python 3 authority server
and fixes its public listener at `0.0.0.0:28782`, `server_random`, 30 players.
The x64/unsigned executable changes distribution only; server state and
protocol v5 remain identical to the source entry point.
...
and armor intersection. It advances every shell through adaptive gravity
chords no longer than 50 ms and with at most 5 cm sagitta, including
moving-vehicle sweeps, and submits the chronological result. The server
validates its envelope and atomically commits one terminal
