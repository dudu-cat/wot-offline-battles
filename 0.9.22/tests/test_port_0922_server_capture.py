...
class ServerCaptureTests(unittest.TestCase):
    ...
    def test_real_erlenberg_graph_sends_packed_objective_bases(self):
        ...
        tactical = self._state()._map_rule_data()['bases']
        self.assertGreater(
            ((accepted[1][0][0] - tactical[1][0]) ** 2 +
             (accepted[1][0][1] - tactical[1][1]) ** 2) ** 0.5,
            500.0)
    ...
    def test_legacy_manifest_without_world_pose_keeps_capture_compatibility(self):
        state = self._state()
        ...
        self.assertNotIn('world_pose', manifest_bot)
        self.assertTrue(state.update_bot_manifest(1, {
            'round_id': state.round_id, 'bots': [manifest_bot],
        }))
        self.assertTrue(state.bot_states[16]['world_pose'])
